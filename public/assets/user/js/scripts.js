...
function initCitybook() {
 ...
 initAutocomplete();
    ...
    modal_input.hide = function () {
        $('.modal-input').fadeOut();
        $("html, body").removeClass("hid-body");
    };
    modal_tipe.hide = function () {
        $('.modal-tipe').fadeOut();
        $("html, body").removeClass("hid-body");
    };
    ...
    $('.modal-input-open').on("click", function (e) {
        e.preventDefault();
        $('.modal-input').fadeIn();
        $("html, body").addClass("hid-body");
    });
    $('.modal-tipe-open').on("click", function (e) {
        e.preventDefault();
        $('.modal-tipe').fadeIn();
        $("html, body").addClass("hid-body");
    });
    ...
    $('.close-reg-tipe').on("click", function () {
        modal_tipe.hide();
    });
	// click ------------------
    ...
    $('input[type="range"].min').rangeslider({
        polyfill: false,
        onInit: function () {
            this.output = $(".min span").html(this.$element.val());
            this.validation = $(".max span").html(this.$element.val());
        },
        onSlide: function (
            position, value) {
            ...
            if (value>max_price) {
                this.validation.html(value);
                document.getElementById("max-price").min = value;
                document.getElementById("max-price").val = value;
            }
        }
    });
    $('input[type="range"].max').rangeslider({
        polyfill: false,
        onInit: function () {
            this.output = $(".max span").html(this.$element.val());
        },
        onSlide: function (
            position, value) {
    ...
    $('select[name=category_properti]').on('change', function(){
        let category = $(this).val();
        if (category=="rumah") {
            var select = '<select class="chosen-select-new" name="category_detail">'+
                          '</select>';
            $('#test').empty();
            $('#test').html(select);
            for (var i = 1; i <= 7; i++) {
                $('select[name=category_detail]').append('<option value="'+ i + '">'+ 'Tipe ' + i + '</option>');
            }
            $('.chosen-select-new').niceSelect();
        }
            ...
            $('#test').html(select);
            for (var i = 1; i <= 7; i++) {
                $('select[name=category_detail]').append('<option value="'+ i + '">'+'Luas 4 x '+i+'</option>');
            }
            $('.chosen-select-new').niceSelect();
        }else{
            var select = '<select class="chosen-select-new" name="category_detail">'+
