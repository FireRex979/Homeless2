            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <button class="btn btn-sm btn-rounded btn-success" data-toggle="modal"
                                    data-target="#addFasilitas">+ Tambah Kelebihan</button>
                    <br>
                    <br>
                                ...
                                @if ($kelebihans)
                                    @foreach ($kelebihans as $kelebihan)
                                     <tr>
                                        ...
                                        <td>{{$kelebihan->satuan}}</td>
                                        <td>
                                            <ul class="list-inline m-0">
                                                <li class="list-inline-item">
                                                    <button class="edit btn btn-success btn-sm rounded-0" type="button" data-toggle="modal" data-target="#editFasilitas" data-placement="top" title="Edit" onclick="showEdit({{$kelebihan->id}})"><i class="fa fa-edit"></i></button>
                                                </li>
                                                <li class="list-inline-item">
                                                    <button class="hapus btn btn-danger btn-sm rounded-0" type="button" data-toggle="tooltip" data-placement="top" title="Delete"><i class="fa fa-trash" onclick="hapus({{$kelebihan->id}},{{$loop->iteration}})"></i></button>
                                                </li>
                                            </ul>
                                        </td>
                                    </tr>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Tambah Kelebihan</h4>
                <button type="button" class="close" data-dismiss="modal"
                    aria-label="Close"> <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{route('kelebihan.store')}}" method="POST">
            <div class="modal-body">
                    @csrf
                    <div class="form-group">
                        <label>Nama Kelebihan</label>
                        <input type="text" class="form-control"
                            placeholder="Masukkan nama kelebihan" required name="kelebihan"></div>
                    <div class="form-group">
                        <label>Nama Satuan</label>
                        <input type="text" class="form-control"
                            placeholder="Masukkan nama satuan" required name="satuan"></div>
            </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label>Nama Kelebihan</label>
                        <input type="text" id="namaKelebihan" class="form-control"
                            placeholder="Masukkan nama kelebihan" required name="kelebihan"></div>
                    <div class="form-group">
                        <label>Nama Satuan</label>
                        <input type="text" id="namaSatuan" class="form-control"
                            placeholder="Masukkan nama satuan" required name="satuan"></div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary"
...
<script>
    function hapus(id_fasilitas, numRow){
        ...
        if(konfirmasi == true){
            jQuery.ajax({
                    url: "/admin/kelebihan/"+id_fasilitas,
                    method: 'delete',
                    data: {
    ...
    function showEdit(id_fasilitas){
        jQuery.ajax({
                url: "/admin/kelebihan/"+id_fasilitas+"/edit",
                method: 'get',
                success: function(result){
                    ...
                    $("#formEdit").attr("action", "/admin/kelebihan/"+id_fasilitas);
                }
        });
    }
