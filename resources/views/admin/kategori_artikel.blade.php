...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h2 class="card-title">Kategori Artikel</h2>
                    <h6 class="card-subtitle"></h6>
                    <br>
                    <button class="btn btn-sm btn-rounded btn-success" data-toggle="modal"
                                    data-target="#addFasilitas">+ Tambah Kategori Artikel</button>
                    <br>
                    <br>
                    <div class="table-responsive">
                        <table id="zero_config" class="table table-striped table-bordered"
                            ...
                            <tbody>
                                @if ($artikels)
                                    @foreach ($artikels as $artikel)
                                     <tr>
                                        ...
                                        <td>{{$artikel->category_name}}</td>
                                        <td>
                                            <ul class="list-inline m-0">
                                                <li class="list-inline-item">
                                                    <button class="edit btn btn-success btn-sm rounded-0" type="button" data-toggle="modal" data-target="#editFasilitas" data-placement="top" title="Edit" onclick="showEdit({{$artikel->id}})"><i class="fa fa-edit"></i></button>
                                                </li>
                                                <li class="list-inline-item">
                                ...
                                @else
                                        <tr>
                                            <td>Belum ada data kategori artikel</td>
                                        </tr>
                                @endif
                            </tbody>
                        </table>
            ...
            <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control"
                            placeholder="Masukkan nama kategori artikel" required name="category_name"></div>
            </div>
...
<script>
    function hapus(id_fasilitas, numRow){
        ...
        if(konfirmasi == true){
            jQuery.ajax({
                    url: "/admin/kategori-artikel/"+id_fasilitas,
                    method: 'delete',
                    data: {
                        _token: $('#signup-token').val(),
                    },
                    success: function(result){
                        $("#pesan").text("Fasilitas "+result.artikel+" telah berhasil dihapus");
                        $("#status-hapus").show();
    ...
    function showEdit(id_fasilitas){
        jQuery.ajax({
                url: "/admin/kategori-artikel/"+id_fasilitas+"/edit",
                method: 'get',
                success: function(result){
                    // $('.ganti').html(result.hasil);
                    // $('#editFasilitas').modal('show');
                    $("#namaFasilitas").val(result.artikel['category_name']);
                    $("#formEdit").attr("action", "/admin/kategori-artikel/"+id_fasilitas);
                }
        });
    }
